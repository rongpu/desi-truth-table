...
FROM
    pdr2_dud.forced main
    LEFT JOIN pdr2_dud.forced2 main2 USING (object_id) 
    -- LEFT JOIN pdr2_dud.photoz_demp pz_demp USING (object_id)
    -- LEFT JOIN pdr2_dud.photoz_ephor pz_ephor USING (object_id)
    -- LEFT JOIN pdr2_dud.photoz_ephor_ab pz_ephor_ab USING (object_id)
    -- LEFT JOIN pdr2_dud.photoz_frankenz pz_frankenz USING (object_id)
    -- LEFT JOIN pdr2_dud.photoz_mizuki pz_mizuki USING (object_id)
    -- LEFT JOIN pdr2_dud.photoz_mlz pz_mlz USING (object_id)
    -- LEFT JOIN pdr2_dud.photoz_nnpz pz_nnpz USING (object_id)

WHERE
    pdr2_dud.search_cosmos(object_id)
    -- pdr2_dud.search_deep2_3(object_id)
    -- pdr2_dud.search_elais_n1(object_id)
    -- pdr2_dud.search_sxds(object_id)
    -- pdr2_dud.search_xmm_lss(object_id)
    AND isprimary
    AND (r_cmodel_mag<25.5 OR g_cmodel_mag<25.5)
