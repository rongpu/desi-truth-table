...
FROM
    ...
    LEFT JOIN pdr3_dud.forced2 main2 USING (object_id) 
    LEFT JOIN pdr3_dud.photoz_demp pz_demp USING (object_id)
    LEFT JOIN pdr3_dud.photoz_mizuki pz_mizuki USING (object_id)
    LEFT JOIN pdr3_dud.photoz_dnnz pz_dnnz USING (object_id)

WHERE
    isprimary
    AND (g_cmodel_mag<25.5 OR r_cmodel_mag<25.5 OR i_cmodel_mag<25.0 OR z_cmodel_mag<24.3)
