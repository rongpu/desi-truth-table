...
data_path = '/global/project/projectdirs/desi/target/analysis/truth/parent/original/'
...
cat = fitsio.read(cat_path)

mask = cat['SPECPRIMARY']==1
print(np.sum(mask), np.sum(mask)/len(mask))
cat = cat[mask]

fitsio.write('/global/project/projectdirs/desi/target/analysis/truth/parent/sdss-specObj-dr16-unique-trimmed.fits', cat)